/*----------------------------------------------------------------------
 *
 * rdb_schcomp -- comparing database schemas
 *
 *  SUPPORTED
 * ~~~~~~~~~~~
...
rdbcompGeneralOptions		options;	/* general options */
...
static void loadConfig(const char *c, rdbcompOptions *o);
static PGconn *connectDatabase(rdbcompDatabaseOptions opt);
...
static void rdbcompAccessMethods();
static void rdbcompAggregates();
static void rdbcompCasts();
static void rdbcompCollations();
static void rdbcompConversions();
static void rdbcompDomains();
static void rdbcompEventTriggers();
static void rdbcompExtensions();
static void rdbcompForeignDataWrappers();
static void rdbcompForeignServers();
static void rdbcompForeignTables();
static void rdbcompFunctions();
static void rdbcompIndexes();
static void rdbcompLanguages();
static void rdbcompMaterializedViews();
static void rdbcompOperators();
static void rdbcompOperatorFamilies();
static void rdbcompOperatorClasses();
static void rdbcompPublications();
static void rdbcompPolicies();
static void rdbcompProcedures();
static void rdbcompRules();
static void rdbcompSchemas();
static void rdbcompSequences();
static void rdbcompStatistics();
static void rdbcompSubscriptions();
static void rdbcompTables();
static void rdbcompTextSearchConfigs();
static void rdbcompTextSearchDicts();
static void rdbcompTextSearchParsers();
static void rdbcompTextSearchTemplates();
static void rdbcompTransforms();
static void rdbcompTriggers();
static void rdbcompBaseTypes();
static void rdbcompCompositeTypes();
static void rdbcompEnumTypes();
static void rdbcompRangeTypes();
static void rdbcompTypes();
static void rdbcompUserMappings();
static void rdbcompViews();
...
static void
loadConfig(const char *cf, rdbcompOptions *options)
{
	MiniFile	*config;
	...
	if (config != NULL)
	{
		...
		for (sec = config->section; sec != NULL; sec = sec->next)
		{
			for (secdata = sec->data; secdata != NULL; secdata = secdata->next)
				logDebug("section: \"%s\" ; key: \"%s\" ; value: \"%s\"", sec->name, secdata->key, secdata->value);
		}
		...
		if (mini_file_get_value(config, "general", "verbose") != NULL)
			options->general.verbose = parseBoolean("verbose", mini_file_get_value(config, "general", "verbose"));

		if (mini_file_get_value(config, "general", "summary") != NULL) {
			options->general.summary = parseBoolean("summary", mini_file_get_value(config, "general", "summary"));
		}

		if (mini_file_get_value(config, "general", "comment") != NULL)
			options->general.comment = parseBoolean("comment", mini_file_get_value(config, "general", "comment"));

		if (mini_file_get_value(config, "general", "security-labels") != NULL)
			options->general.securitylabels = parseBoolean("security-labels", mini_file_get_value(config, "general", "security-labels"));

		if (mini_file_get_value(config, "general", "owner") != NULL)
			options->general.owner = parseBoolean("owner", mini_file_get_value(config, "general", "owner"));

		if (mini_file_get_value(config, "general", "privileges") != NULL)
			options->general.privileges = parseBoolean("privileges", mini_file_get_value(config, "general", "privileges"));

		if (mini_file_get_value(config, "general", "ignore-version") != NULL)
			options->general.ignoreversion = parseBoolean("ignore-version", mini_file_get_value(config, "general", "ignore-version"));

		if (mini_file_get_value(config, "general", "single-transaction") != NULL)
			options->general.singletxn = parseBoolean("single-transaction", mini_file_get_value(config, "general", "single-transaction"));

		/*
		 * select objects that will be compared
		 */
		if (mini_file_get_value(config, "general", "access-method") != NULL)
			options->general.accessmethod = parseBoolean("access-method", mini_file_get_value(config, "general", "access-method"));

		if (mini_file_get_value(config, "general", "aggregate") != NULL)
			options->general.aggregate = parseBoolean("aggregate", mini_file_get_value(config, "general", "aggregate"));

		if (mini_file_get_value(config, "general", "cast") != NULL)
			options->general.cast = parseBoolean("cast", mini_file_get_value(config, "general", "cast"));

		if (mini_file_get_value(config, "general", "collation") != NULL)
			options->general.collation = parseBoolean("collation", mini_file_get_value(config, "general", "collation"));

		if (mini_file_get_value(config, "general", "conversion") != NULL)
			options->general.conversion = parseBoolean("conversion", mini_file_get_value(config, "general", "conversion"));

		if (mini_file_get_value(config, "general", "domain") != NULL)
			options->general.domain = parseBoolean("domain", mini_file_get_value(config, "general", "domain"));

		if (mini_file_get_value(config, "general", "event-trigger") != NULL)
			options->general.eventtrigger = parseBoolean("event-trigger", mini_file_get_value(config, "general", "event-trigger"));

		if (mini_file_get_value(config, "general", "extension") != NULL)
			options->general.extension = parseBoolean("extension", mini_file_get_value(config, "general", "extension"));

		if (mini_file_get_value(config, "general", "fdw") != NULL)
			options->general.fdw = parseBoolean("fdw", mini_file_get_value(config, "general", "fdw"));

		if (mini_file_get_value(config, "general", "foreign-table") != NULL)
			options->general.foreigntable = parseBoolean("foreign-table", mini_file_get_value(config, "general", "foreign-table"));

		if (mini_file_get_value(config, "general", "function") != NULL)
			options->general.function = parseBoolean("function", mini_file_get_value(config, "general", "function"));

		if (mini_file_get_value(config, "general", "index") != NULL)
			options->general.index = parseBoolean("index", mini_file_get_value(config, "general", "index"));

		if (mini_file_get_value(config, "general", "language") != NULL)
			options->general.language = parseBoolean("language", mini_file_get_value(config, "general", "language"));

		if (mini_file_get_value(config, "general", "materialized-view") != NULL)
			options->general.matview = parseBoolean("materialized-view", mini_file_get_value(config, "general", "materialized-view"));

		if (mini_file_get_value(config, "general", "operator") != NULL)
			options->general.operator = parseBoolean("operator", mini_file_get_value(config, "general", "operator"));

		if (mini_file_get_value(config, "general", "policy") != NULL)
			options->general.policy = parseBoolean("policy", mini_file_get_value(config, "general", "policy"));

		if (mini_file_get_value(config, "general", "procedure") != NULL)
			options->general.procedure = parseBoolean("procedure", mini_file_get_value(config, "general", "procedure"));

		if (mini_file_get_value(config, "general", "publication") != NULL)
			options->general.publication = parseBoolean("publication", mini_file_get_value(config, "general", "publication"));

		if (mini_file_get_value(config, "general", "rule") != NULL)
			options->general.rule = parseBoolean("rule", mini_file_get_value(config, "general", "rule"));

		if (mini_file_get_value(config, "general", "schema") != NULL)
			options->general.schema = parseBoolean("schema", mini_file_get_value(config, "general", "schema"));

		if (mini_file_get_value(config, "general", "sequence") != NULL)
			options->general.sequence = parseBoolean("sequence", mini_file_get_value(config, "general", "sequence"));

		if (mini_file_get_value(config, "general", "statistics") != NULL)
			options->general.statistics = parseBoolean("statistics", mini_file_get_value(config, "general", "statistics"));

		if (mini_file_get_value(config, "general", "subscription") != NULL)
			options->general.subscription = parseBoolean("subscription", mini_file_get_value(config, "general", "subscription"));

		if (mini_file_get_value(config, "general", "table") != NULL)
			options->general.table = parseBoolean("table", mini_file_get_value(config, "general", "table"));

		if (mini_file_get_value(config, "general", "text-search") != NULL)
			options->general.textsearch = parseBoolean("text-search", mini_file_get_value(config, "general", "text-search"));

		if (mini_file_get_value(config, "general", "transform") != NULL)
			options->general.transform = parseBoolean("transform", mini_file_get_value(config, "general", "transform"));

		if (mini_file_get_value(config, "general", "trigger") != NULL)
			options->general.trigger = parseBoolean("trigger", mini_file_get_value(config, "general", "trigger"));

		if (mini_file_get_value(config, "general", "type") != NULL)
			options->general.type = parseBoolean("type", mini_file_get_value(config, "general", "type"));

		if (mini_file_get_value(config, "general", "view") != NULL)
			options->general.view = parseBoolean("view", mini_file_get_value(config, "general", "view"));
		...
		else
		{
			...
			if (tmp != NULL) options->target.password = strdup(tmp);
		}

		tmp = mini_file_get_value(config, "target", "dbname");
		if (tmp != NULL) options->target.dbname = strdup(tmp);
		else
		{
		...
		if (tmp != NULL) options->target.promptpassword = !parseBoolean("no-password", mini_file_get_value(config, "target", "no-password"));
	}
	else
	{
...
static PGconn *
connectDatabase(rdbcompDatabaseOptions opt)
{
	PGconn		*conn;
...
#define NUMBER_OF_PARAMS	7
	...
	{
		if (opt.istarget)
			logError("connection to target database failed: %s", PQerrorMessage(conn));
		else
			logError("connection to source database failed: %s", PQerrorMessage(conn));
		PQfinish(conn);
		exit(EXIT_FAILURE);
	}
...
rdbcompAccessMethods()
{
	...
	logLine1("ACCESS METHODS COMPARE");

	logLine2("Access Methods on server1");
	ams1 = getAccessMethods(conn1, &nams1);

	logLine2("Access Methods on server2");
	ams2 = getAccessMethods(conn2, &nams2);

	for (i = 0; i < nams1; i++) logNoise("server1: %s", ams1[i].amname);
	for (i = 0; i < nams2; i++) logNoise("server2: %s", ams2[i].amname);
...
rdbcompAggregates()
{
	...
	logLine1("COMPARE AGGREGATES");

	logLine2("Aggregates on server1");
	aggregates1 = getAggregates(conn1, &naggregates1);

	logLine2("Aggregates on server2");
	aggregates2 = getAggregates(conn2, &naggregates2);

	for (i = 0; i < naggregates1; i++) logNoise("server1: %s.%s(%s)", aggregates1[i].obj.schemaname, aggregates1[i].obj.objectname, aggregates1[i].arguments);
	for (i = 0; i < naggregates2; i++) logNoise("server2: %s.%s(%s)", aggregates2[i].obj.schemaname, aggregates2[i].obj.objectname, aggregates2[i].arguments);
...
rdbcompCasts()
{
	...
	logLine1("COMPARE CASTS");

	logLine2("Casts on server1");
	casts1 = getCasts(conn1, &ncasts1);

	logLine2("Casts on server2");
	casts2 = getCasts(conn2, &ncasts2);

	for (i = 0; i < ncasts1; i++) logNoise("server1: cast %s AS %s", casts1[i].source, casts1[i].target);
	for (i = 0; i < ncasts2; i++) logNoise("server2: cast %s AS %s", casts2[i].source, casts2[i].target);
...
rdbcompCollations()
{
	...
	/* Collations */
	logLine1("COMPARE COLLATIONS");

	logLine2("Collations on server1");
	collations1 = getCollations(conn1, &ncollations1);

	logLine2("Collations on server2");
	collations2 = getCollations(conn2, &ncollations2);

	for (i = 0; i < ncollations1; i++) logNoise("server1: %s.%s", collations1[i].obj.schemaname, collations1[i].obj.objectname);
	for (i = 0; i < ncollations2; i++) logNoise("server2: %s.%s", collations2[i].obj.schemaname, collations2[i].obj.objectname);
	...
	while (i < ncollations1 || j < ncollations2)
	{
		...
		if (i == ncollations1)
		{
			logDebug("collation %s.%s: server2", collations2[j].obj.schemaname, collations2[j].obj.objectname);
		...
		{
			logDebug("collation %s.%s: server1", collations1[i].obj.schemaname, collations1[i].obj.objectname);
		...
		{
			logDebug("collation %s.%s: server1 server2", collations1[i].obj.schemaname, collations1[i].obj.objectname);
...
rdbcompConversions()
{
	...
	logLine1("COMPARE CONVERSIONS");

	logLine2("Conversions on server1");
	conversions1 = getConversions(conn1, &nconversions1);

	logLine2("Conversions on server2");
	conversions2 = getConversions(conn2, &nconversions2);

	for (i = 0; i < nconversions1; i++) logNoise("server1: %s.%s %u", conversions1[i].obj.schemaname, conversions1[i].obj.objectname, conversions1[i].obj.oid);
	for (i = 0; i < nconversions2; i++) logNoise("server2: %s.%s %u", conversions2[i].obj.schemaname, conversions2[i].obj.objectname, conversions2[i].obj.oid);
	...
	while (i < nconversions1 || j < nconversions2)
	{
		...
		if (i == nconversions1)
		{
			logDebug("conversion %s.%s: server2", conversions2[j].obj.schemaname, conversions2[j].obj.objectname);
		...
		{
			logDebug("conversion %s.%s: server1", conversions1[i].obj.schemaname, conversions1[i].obj.objectname);
		...
		{
			logDebug("conversion %s.%s: server1 server2", conversions1[i].obj.schemaname, conversions1[i].obj.objectname);
...
rdbcompDomains()
{
	...
	/* Domains */
	logLine1("COMPARE DOMAINS");
	logLine2("Domains on server1");
	domains1 = getDomains(conn1, &ndomains1);
	logLine2("Domains on server2");
	domains2 = getDomains(conn2, &ndomains2);

	for (i = 0; i < ndomains1; i++) logNoise("server1: %s.%s", domains1[i].obj.schemaname, domains1[i].obj.objectname);
	for (i = 0; i < ndomains2; i++) logNoise("server2: %s.%s", domains2[i].obj.schemaname, domains2[i].obj.objectname);
	...
	while (i < ndomains1 || j < ndomains2)
	{
		...
		if (i == ndomains1)
		{
			logDebug("domain %s.%s: server2", domains2[j].obj.schemaname, domains2[j].obj.objectname);
			getDomainConstraints(conn2, &domains2[j]);
			if (options.securitylabels) getDomainSecurityLabels(conn2, &domains2[j]);
		...
		{
			logDebug("domain %s.%s: server1", domains1[i].obj.schemaname, domains1[i].obj.objectname);
		...
		{
			logDebug("domain %s.%s: server1 server2", domains1[i].obj.schemaname, domains1[i].obj.objectname);
...
rdbcompEventTriggers()
{
	...
	logLine1("COMPARE TRIGGERS");
	logLine2("Triggers on server1");
	evttrgs1 = getEventTriggers(conn1, &nevttrgs1);
	logLine2("Triggers on server2");
	evttrgs2 = getEventTriggers(conn2, &nevttrgs2);

	for (i = 0; i < nevttrgs1; i++) logNoise("server1: %s", evttrgs1[i].trgname);
	for (i = 0; i < nevttrgs2; i++) logNoise("server2: %s", evttrgs2[i].trgname);
	...
	while (i < nevttrgs1 || j < nevttrgs2)
	{
		...
		if (i == nevttrgs1)
		{
			...
			if (options.securitylabels) getEventTriggerSecurityLabels(conn2, &evttrgs2[j]);
...
rdbcompExtensions()
{
	...
	logLine1("COMPARE Extensions");
	logLine2("Extensions on server1");
	extensions1 = getExtensions(conn1, &nextensions1);
	logLine2("Extensions on server2");
	extensions2 = getExtensions(conn2, &nextensions2);

	for (i = 0; i < nextensions1; i++) logNoise("server1: %s", extensions1[i].extensionname);
	for (i = 0; i < nextensions2; i++) logNoise("server2: %s", extensions2[i].extensionname);
...
rdbcompForeignDataWrappers()
{
	...
	logLine1("COMPARE Foreign Data Wrappers");
	logLine2("Foreign Data Wrappers on server1");
	fdws1 = getForeignDataWrappers(conn1, &nfdws1);
	logLine2("Foreign Data Wrappers on server2");
	fdws2 = getForeignDataWrappers(conn2, &nfdws2);

	for (i = 0; i < nfdws1; i++) logNoise("server1: %s", fdws1[i].fdwname);
	for (i = 0; i < nfdws2; i++) logNoise("server2: %s", fdws2[i].fdwname);
...
rdbcompForeignServers()
{
	...
	logLine1("COMPARE Foreign Servers");
	logLine2("Foreign Servers on server1");
	servers1 = getForeignServers(conn1, &nservers1);
	logLine2("Foreign Servers on server2");
	servers2 = getForeignServers(conn2, &nservers2);

	for (i = 0; i < nservers1; i++) logNoise("server1: %s", servers1[i].servername);
	for (i = 0; i < nservers2; i++) logNoise("server2: %s", servers2[i].servername);
...
rdbcompFunctions()
{
	...
	logLine1("COMPARE Functions");
	logLine2("Functions on server1");
	functions1 = getFunctions(conn1, &nfunctions1);
	logLine2("Functions on server2");
	functions2 = getFunctions(conn2, &nfunctions2);

	for (i = 0; i < nfunctions1; i++)
		logNoise("server1: %s.%s(%s) %s", functions1[i].obj.schemaname, functions1[i].obj.objectname, functions1[i].arguments, functions1[i].returntype);

	for (i = 0; i < nfunctions2; i++)
		logNoise("server2: %s.%s(%s) %s", functions2[i].obj.schemaname, functions2[i].obj.objectname, functions2[i].arguments, functions2[i].returntype);
	...
	while (i < nfunctions1 || j < nfunctions2)
	{
		...
		if (i == nfunctions1)
		{
			logDebug("function %s.%s(%s): server2", functions2[j].obj.schemaname, functions2[j].obj.objectname, functions2[j].arguments);
			if (options.securitylabels) getFunctionSecurityLabels(conn2, &functions2[j]);
		...
		{
			logDebug("function %s.%s(%s): server1", functions1[i].obj.schemaname, functions1[i].obj.objectname, functions1[i].arguments);
		...
		{
			logDebug("function %s.%s(%s): server1 server2", functions1[i].obj.schemaname, functions1[i].obj.objectname, functions1[i].arguments);
			if (options.securitylabels)
			{
...
rdbcompIndexes()
{
	...
	logLine1("COMPARE Indexes");
	logLine2("Indexes on server1");
	indexes1 = getIndexes(conn1, &nindexes1);
	logLine2("Indexes on server2");
	indexes2 = getIndexes(conn2, &nindexes2);

	for (i = 0; i < nindexes1; i++) logNoise("server1: %s.%s", indexes1[i].obj.schemaname, indexes1[i].obj.objectname);
	for (i = 0; i < nindexes2; i++) logNoise("server2: %s.%s", indexes2[i].obj.schemaname, indexes2[i].obj.objectname);
	...
	while (i < nindexes1 || j < nindexes2)
	{
		...
		if (i == nindexes1)
		{
			logDebug("index %s.%s: server2", indexes2[j].obj.schemaname, indexes2[j].obj.objectname);
		...
		{
			logDebug("index %s.%s: server1", indexes1[i].obj.schemaname, indexes1[i].obj.objectname);
		...
		{
			logDebug("index %s.%s: server1 server2", indexes1[i].obj.schemaname, indexes1[i].obj.objectname);
...
rdbcompLanguages()
{
	...
	logLine1("COMPARE Languages");
	logLine2("Languages on server1");
	languages1 = getLanguages(conn1, &nlanguages1);
	logLine2("Languages on server2");
	languages2 = getLanguages(conn2, &nlanguages2);

	for (i = 0; i < nlanguages1; i++) logNoise("server1: %s", languages1[i].languagename);
	for (i = 0; i < nlanguages2; i++) logNoise("server2: %s", languages2[i].languagename);
	...
	while (i < nlanguages1 || j < nlanguages2)
	{
		...
		if (i == nlanguages1)
		{
			...
			if (options.securitylabels) getLanguageSecurityLabels(conn2, &languages2[j]);
...
rdbcompMaterializedViews()
{
	...
	logLine1("COMPARE Materialized Views");
	logLine2("Materialized Views on server1");
	matviews1 = getMaterializedViews(conn1, &nmatviews1);
	logLine2("Materialized Views on server2");
	matviews2 = getMaterializedViews(conn2, &nmatviews2);

	for (i = 0; i < nmatviews1; i++) logNoise("server1: %s.%s", matviews1[i].obj.schemaname, matviews1[i].obj.objectname);
	for (i = 0; i < nmatviews2; i++) logNoise("server2: %s.%s", matviews2[i].obj.schemaname, matviews2[i].obj.objectname);
	...
	while (i < nmatviews1 || j < nmatviews2)
	{
		...
		if (i == nmatviews1)
		{
			logDebug("materialized view %s.%s: server2", matviews2[j].obj.schemaname, matviews2[j].obj.objectname);
			getMaterializedViewAttributes(conn2, &matviews2[j]);
			if (options.securitylabels) getMaterializedViewSecurityLabels(conn2, &matviews2[j]);
		...
		{
			logDebug("materialized view %s.%s: server1", matviews1[i].obj.schemaname, matviews1[i].obj.objectname);
		...
		{
			logDebug("materialized view %s.%s: server1 server2", matviews1[i].obj.schemaname, matviews1[i].obj.objectname);
...
rdbcompOperators()
{
	...
	logLine1("COMPARE Operators");
	logLine2("Operators on server1");
	operators1 = getOperators(conn1, &noperators1);
	logLine2("Operators on server2");
	operators2 = getOperators(conn2, &noperators2);

	for (i = 0; i < noperators1; i++)
		logNoise("server1: %s.%s", operators1[i].obj.schemaname, operators1[i].obj.objectname);

	for (i = 0; i < noperators2; i++)
		logNoise("server2: %s.%s", operators2[i].obj.schemaname, operators2[i].obj.objectname);
	...
	while (i < noperators1 || j < noperators2)
	{
		...
		if (i == noperators1)
		{
			logDebug("operator %s.%s: server2", operators2[j].obj.schemaname, operators2[j].obj.objectname);
		...
		{
			logDebug("operator %s.%s: server1", operators1[i].obj.schemaname, operators1[i].obj.objectname);
		...
		{
			logDebug("operator %s.%s: server1 server2", operators1[i].obj.schemaname, operators1[i].obj.objectname);
...
rdbcompOperatorFamilies()
{
	...
	logLine1("COMPARE Operator Families");
	logLine2("Operator Families on server1");
	opfamilies1 = getOperatorFamilies(conn1, &nopfamilies1);
	logLine2("Operator Families on server2");
	opfamilies2 = getOperatorFamilies(conn2, &nopfamilies2);

	for (i = 0; i < nopfamilies1; i++) logNoise("server1: %s.%s", opfamilies1[i].obj.schemaname, opfamilies1[i].obj.objectname);
	for (i = 0; i < nopfamilies2; i++) logNoise("server2: %s.%s", opfamilies2[i].obj.schemaname, opfamilies2[i].obj.objectname);
	...
	while (i < nopfamilies1 || j < nopfamilies2)
	{
		...
		if (i == nopfamilies1)
		{
			logDebug("operator family %s.%s: server2", opfamilies2[j].obj.schemaname, opfamilies2[j].obj.objectname);
		...
		{
			logDebug("operator family %s.%s: server1", opfamilies1[i].obj.schemaname, opfamilies1[i].obj.objectname);
		...
		{
			logDebug("operator family %s.%s: server1 server2", opfamilies1[i].obj.schemaname, opfamilies1[i].obj.objectname);
...
rdbcompOperatorClasses()
{
	...
	logLine1("COMPARE Operator Classes");
	logLine2("Operator Classes on server1");
	opclasses1 = getOperatorClasses(conn1, &nopclasses1);
	logLine2("Operator Classes on server2");
	opclasses2 = getOperatorClasses(conn2, &nopclasses2);

	for (i = 0; i < nopclasses1; i++) logNoise("server1: %s.%s", opclasses1[i].obj.schemaname, opclasses1[i].obj.objectname);
	for (i = 0; i < nopclasses2; i++) logNoise("server2: %s.%s", opclasses2[i].obj.schemaname, opclasses2[i].obj.objectname);
	...
	while (i < nopclasses1 || j < nopclasses2)
	{
		...
		if (i == nopclasses1)
		{
			logDebug("operator class %s.%s: server2", opclasses2[j].obj.schemaname, opclasses2[j].obj.objectname);
		...
		{
			logDebug("operator class %s.%s: server1", opclasses1[i].obj.schemaname, opclasses1[i].obj.objectname);
		...
		{
			logDebug("operator class %s.%s: server1 server2", opclasses1[i].obj.schemaname, opclasses1[i].obj.objectname);
...
rdbcompProcedures()
{
	...
	logLine1("COMPARE Procedures");
	logLine2("Procedures on server1");
	procedures1 = getProcedures(conn1, &nprocedures1);
	logLine2("Procedures on server2");
	...
	while (i < nprocedures1 || j < nprocedures2)
	{
		...
		if (i == nprocedures1)
		{
			logDebug("procedure %s.%s(%s): server2", procedures2[j].obj.schemaname, procedures2[j].obj.objectname, procedures2[j].arguments);
			if (options.securitylabels) getProcedureSecurityLabels(conn2, &procedures2[j]);
		...
		{
			logDebug("procedure %s.%s(%s): server1", procedures1[i].obj.schemaname, procedures1[i].obj.objectname, procedures1[i].arguments);
		...
		{
			logDebug("procedure %s.%s(%s): server1 server2", procedures1[i].obj.schemaname, procedures1[i].obj.objectname, procedures1[i].arguments);
			if (options.securitylabels)
			{
...
rdbcompPublications()
{
	...
	logLine1("COMPARE Publications");
	logLine2("Publications on server1");
	publications1 = getPublications(conn1, &npublications1);
	logLine2("Publications on server2");
	publications2 = getPublications(conn2, &npublications2);

	for (i = 0; i < npublications1; i++) logNoise("server1: %s", publications1[i].pubname);
	for (i = 0; i < npublications2; i++) logNoise("server2: %s", publications2[i].pubname);
	...
	while (i < npublications1 || j < npublications2)
	{
		...
		if (i == npublications1)
		{
			...
			if (options.securitylabels) getPublicationSecurityLabels(conn2, &publications2[j]);
...
rdbcompPolicies()
{
	...
	logLine1("COMPARE Policies");
	logLine2("Policies on server1");
	policies1 = getPolicies(conn1, &npolicies1);
	logLine2("Policies on server2");
	policies2 = getPolicies(conn2, &npolicies2);

	for (i = 0; i < npolicies1; i++) logNoise("server1: %s.%s", policies1[i].table.schemaname, policies1[i].table.objectname);
	for (i = 0; i < npolicies2; i++) logNoise("server2: %s.%s", policies2[i].table.schemaname, policies2[i].table.objectname);
	...
	while (i < npolicies1 || j < npolicies2)
	{
		...
		if (i == npolicies1)
		{
			logDebug("policy %s.%s: server2", policies2[j].table.schemaname, policies2[j].table.objectname);
		...
		{
			logDebug("policy %s.%s: server1", policies1[i].table.schemaname, policies1[i].table.objectname);
			dumpDropPolicy(fpost, &policies1[i]);
			i++;
			qstat.polremoved++;
		}
		else if (compareNamesAndRelations(&policies1[i].table, &policies2[j].table, policies1[i].polname, policies2[j].polname) == 0)
		{
			logDebug("policy %s.%s: server1 server2", policies1[i].table.schemaname, policies1[i].table.objectname);
...
rdbcompRules()
{
	...
	logLine1("COMPARE Roles");
	logLine2("Roles on server1");
	rules1 = getRules(conn1, &nrules1);
	logLine2("Roles on server2");
	rules2 = getRules(conn2, &nrules2);

	for (i = 0; i < nrules1; i++) logNoise("server1: %s.%s", rules1[i].table.schemaname, rules1[i].table.objectname);
	for (i = 0; i < nrules2; i++) logNoise("server2: %s.%s", rules2[i].table.schemaname, rules2[i].table.objectname);
	...
	while (i < nrules1 || j < nrules2)
	{
		...
		if (i == nrules1)
		{
			logDebug("rule %s.%s: server2", rules2[j].table.schemaname, rules2[j].table.objectname);
		...
		{
			logDebug("rule %s.%s: server1", rules1[i].table.schemaname, rules1[i].table.objectname);
		...
		{ 
			logDebug("rule %s.%s: server1 server2", rules1[i].table.schemaname, rules1[i].table.objectname);
...
rdbcompSchemas()
{
	...
	logLine1("COMPARE Schemas");
	logLine2("Schemas on server1");
	schemas1 = getSchemas(conn1, &nschemas1);
	logLine2("Schemas on server2");
	schemas2 = getSchemas(conn2, &nschemas2);

	for (i = 0; i < nschemas1; i++) logNoise("server1: %s", schemas1[i].schemaname);
	for (i = 0; i < nschemas2; i++) logNoise("server2: %s", schemas2[i].schemaname);
	...
	while (i < nschemas1 || j < nschemas2)
	{
		...
		if (i == nschemas1)
		{
			...
			if (options.securitylabels) getSchemaSecurityLabels(conn2, &schemas2[j]);
...
rdbcompSequences()
{
	...
	logLine1("COMPARE Sequences");
	logLine2("Sequences on server1");
	sequences1 = getSequences(conn1, &nsequences1);
	logLine2("Sequences on server2");
	sequences2 = getSequences(conn2, &nsequences2);

	for (i = 0; i < nsequences1; i++) logNoise("server1: %s.%s", sequences1[i].obj.schemaname, sequences1[i].obj.objectname);
	for (i = 0; i < nsequences2; i++) logNoise("server2: %s.%s", sequences2[i].obj.schemaname, sequences2[i].obj.objectname);
	...
	while (i < nsequences1 || j < nsequences2)
	{
		...
		if (i == nsequences1)
		{
			logDebug("sequence %s.%s: server2", sequences2[j].obj.schemaname, sequences2[j].obj.objectname);
			getSequenceAttributes(conn2, &sequences2[j]);
			if (options.securitylabels) getSequenceSecurityLabels(conn2, &sequences2[j]);
		...
		{
			logDebug("sequence %s.%s: server1", sequences1[i].obj.schemaname, sequences1[i].obj.objectname);
		...
		{
			logDebug("sequence %s.%s: server1 server2", sequences1[i].obj.schemaname, sequences1[i].obj.objectname);
...
rdbcompStatistics()
{
	...
	logLine1("COMPARE Statistics");
	logLine2("Statistics on server1");
	statistics1 = getStatistics(conn1, &nstatistics1);
	logLine2("Statistics on server2");
	statistics2 = getStatistics(conn2, &nstatistics2);

	for (i = 0; i < nstatistics1; i++) logNoise("server1: %s.%s", statistics1[i].obj.schemaname, statistics1[i].obj.objectname);
	for (i = 0; i < nstatistics2; i++) logNoise("server2: %s.%s", statistics2[i].obj.schemaname, statistics2[i].obj.objectname);
	...
	while (i < nstatistics1 || j < nstatistics2)
	{
		...
		if (i == nstatistics1)
		{
			logDebug("statistics %s.%s: server2", statistics2[j].obj.schemaname, statistics2[j].obj.objectname);
		...
		{
			logDebug("statistics %s.%s: server1", statistics1[i].obj.schemaname, statistics1[i].obj.objectname);
		...
		{
			logDebug("statistics %s.%s: server1 server2", statistics1[i].obj.schemaname, statistics1[i].obj.objectname);
...
rdbcompSubscriptions()
{
	...
	logLine1("COMPARE Subscriptions");
	logLine2("Subscriptions on server1");
	subscriptions1 = getSubscriptions(conn1, &nsubscriptions1);
	logLine2("Subscriptions on server2");
	subscriptions2 = getSubscriptions(conn2, &nsubscriptions2);

	for (i = 0; i < nsubscriptions1; i++) logNoise("server1: %s", subscriptions1[i].subname);
	for (i = 0; i < nsubscriptions2; i++) logNoise("server2: %s", subscriptions2[i].subname);
	...
	while (i < nsubscriptions1 || j < nsubscriptions2)
	{
		...
		if (i == nsubscriptions1)
		{
			...
			if (options.securitylabels) getSubscriptionSecurityLabels(conn2, &subscriptions2[j]);
...
rdbcompForeignTables()
{
	...
	logLine1("COMPARE Foreign Tables");
	logLine2("Foreign Tables on server1");
	...
	logLine2("Foreign Tables on server2");
	...
	for (i = 0; i < ntables1; i++) logNoise("server1: %s.%s %u", tables1[i].obj.schemaname, tables1[i].obj.objectname, tables1[i].obj.oid);
	for (i = 0; i < ntables2; i++) logNoise("server2: %s.%s %u", tables2[i].obj.schemaname, tables2[i].obj.objectname, tables2[i].obj.oid);
	...
	while (i < ntables1 || j < ntables2)
	{
		...
		if (i == ntables1)
		{
			logDebug("foreign table %s.%s: server2", tables2[j].obj.schemaname, tables2[j].obj.objectname);
			getTableAttributes(conn2, &tables2[j]);
			if (options.securitylabels) getTableSecurityLabels(conn2, &tables2[j]);
		...
		{
			logDebug("foreign table %s.%s: server1", tables1[i].obj.schemaname, tables1[i].obj.objectname);
		...
		{
			logDebug("foreign table %s.%s: server1 server2", tables1[i].obj.schemaname, tables1[i].obj.objectname);
...
rdbcompTables()
{
	...
	logLine1("COMPARE Tables");

	logLine2("Tables on server1");
	tables1 = getRegularTables(conn1, &ntables1);
	logLine2("Check Constraints on server1");
	getCheckConstraints(conn1, tables1, ntables1);
	logLine2("Foreign Key Constraints on server1");
	getFKConstraints(conn1, tables1, ntables1);
	logLine2("Primary Key Constraints on server1");
	getPKConstraints(conn1, tables1, ntables1);

	logLine2("Tables on server2");
	tables2 = getRegularTables(conn2, &ntables2);
	logLine2("Check Constraints on server1");
	getCheckConstraints(conn2, tables2, ntables2);
	logLine2("Foreign Key Constraints on server2");
	getFKConstraints(conn2, tables2, ntables2);
	logLine2("Primary Key Constraints on server2");
	...
	while (i < ntables1 || j < ntables2)
	{
		...
		if (i == ntables1)
		{
			logDebug("table %s.%s: server2", tables2[j].obj.schemaname, tables2[j].obj.objectname);
			getTableAttributes(conn2, &tables2[j]);
		...
		{
			logDebug("table %s.%s: server1", tables1[i].obj.schemaname, tables1[i].obj.objectname);
			dumpDropTable(fpost, &tables1[i]);
			i++;
			qstat.tableremoved++;
		}
		...
		{
			logDebug("table %s.%s: server1 server2", tables1[i].obj.schemaname, tables1[i].obj.objectname);
			getTableAttributes(conn1, &tables1[i]);
			getTableAttributes(conn2, &tables2[j]);
...
rdbcompTextSearchConfigs()
{
	...
	/* TextSearchConfigs */
	logLine1("COMPARE Text Search Configs");
	logLine2("Text Search Configs on server1");
	tsconfigs1 = getTextSearchConfigs(conn1, &ntsconfigs1);
	logLine2("Text Search Configs on server2");
	tsconfigs2 = getTextSearchConfigs(conn2, &ntsconfigs2);

	for (i = 0; i < ntsconfigs1; i++) logNoise("server1: %s.%s", tsconfigs1[i].obj.schemaname, tsconfigs1[i].obj.objectname);
	for (i = 0; i < ntsconfigs2; i++) logNoise("server2: %s.%s", tsconfigs2[i].obj.schemaname, tsconfigs2[i].obj.objectname);
	...
	while (i < ntsconfigs1 || j < ntsconfigs2)
	{
		...
		if (i == ntsconfigs1)
		{
			logDebug("text search config %s.%s: server2", tsconfigs2[j].obj.schemaname, tsconfigs2[j].obj.objectname);
		...
		{
			logDebug("text search config %s.%s: server1", tsconfigs1[i].obj.schemaname, tsconfigs1[i].obj.objectname);
		...
		{
			logDebug("text search config %s.%s: server1 server2", tsconfigs1[i].obj.schemaname, tsconfigs1[i].obj.objectname);
...
rdbcompTextSearchDicts()
{
	...
	/* TextSearchDicts */
	logLine1("COMPARE Text Search Dictionaries");
	logLine2("Text Search Dictionaries on server1");
	tsdicts1 = getTextSearchDicts(conn1, &ntsdicts1);
	logLine2("Text Search Dictionaries on server2");
	tsdicts2 = getTextSearchDicts(conn2, &ntsdicts2);

	for (i = 0; i < ntsdicts1; i++) logNoise("server1: %s.%s", tsdicts1[i].obj.schemaname, tsdicts1[i].obj.objectname);
	for (i = 0; i < ntsdicts2; i++) logNoise("server2: %s.%s", tsdicts2[i].obj.schemaname, tsdicts2[i].obj.objectname);
	...
	while (i < ntsdicts1 || j < ntsdicts2)
	{
		...
		if (i == ntsdicts1)
		{
			logDebug("text search dictionary %s.%s: server2", tsdicts2[j].obj.schemaname, tsdicts2[j].obj.objectname);
		...
		{
			logDebug("text search dictionary %s.%s: server1", tsdicts1[i].obj.schemaname, tsdicts1[i].obj.objectname);
		...
		{
			logDebug("text search dictionary %s.%s: server1 server2", tsdicts1[i].obj.schemaname, tsdicts1[i].obj.objectname);
...
rdbcompTextSearchParsers()
{
	...
	/* TextSearchParsers */
	logLine1("COMPARE Text Search Parsers");
	logLine2("Text Search Parsers on server1");
	tsparsers1 = getTextSearchParsers(conn1, &ntsparsers1);
	logLine2("Text Search Parsers on server2");
	tsparsers2 = getTextSearchParsers(conn2, &ntsparsers2);

	for (i = 0; i < ntsparsers1; i++) logNoise("server1: %s.%s", tsparsers1[i].obj.schemaname, tsparsers1[i].obj.objectname);
	for (i = 0; i < ntsparsers2; i++) logNoise("server2: %s.%s", tsparsers2[i].obj.schemaname, tsparsers2[i].obj.objectname);
	...
	while (i < ntsparsers1 || j < ntsparsers2)
	{
		...
		if (i == ntsparsers1)
		{
			logDebug("text search parser %s.%s: server2", tsparsers2[j].obj.schemaname, tsparsers2[j].obj.objectname);
		...
		{
			logDebug("text search parser %s.%s: server1", tsparsers1[i].obj.schemaname, tsparsers1[i].obj.objectname);
		...
		{
			logDebug("text search parser %s.%s: server1 server2", tsparsers1[i].obj.schemaname, tsparsers1[i].obj.objectname);
...
rdbcompTextSearchTemplates()
{
	...
	/* TextSearchTemplates */
	logLine1("COMPARE Text Search Templates");
	logLine2("Text Search Templates on server1");
	tstemplates1 = getTextSearchTemplates(conn1, &ntstemplates1);
	logLine2("Text Search Templates on server2");
	tstemplates2 = getTextSearchTemplates(conn2, &ntstemplates2);

	for (i = 0; i < ntstemplates1; i++) logNoise("server1: %s.%s", tstemplates1[i].obj.schemaname, tstemplates1[i].obj.objectname);
	for (i = 0; i < ntstemplates2; i++) logNoise("server2: %s.%s", tstemplates2[i].obj.schemaname, tstemplates2[i].obj.objectname);
	...
	while (i < ntstemplates1 || j < ntstemplates2)
	{
		...
		if (i == ntstemplates1)
		{
			logDebug("text search template %s.%s: server2", tstemplates2[j].obj.schemaname, tstemplates2[j].obj.objectname);
		...
		{
			logDebug("text search template %s.%s: server1", tstemplates1[i].obj.schemaname, tstemplates1[i].obj.objectname);
		...
		{
			logDebug("text search template %s.%s: server1 server2", tstemplates1[i].obj.schemaname, tstemplates1[i].obj.objectname);
...
rdbcompTransforms()
{
	...
	logLine1("COMPARE Transorms");
	logLine2("Transorms on server1");
	transforms1 = getTransforms(conn1, &ntransforms1);
	logLine2("Transorms on server2");
	transforms2 = getTransforms(conn2, &ntransforms2);

	for (i = 0; i < ntransforms1; i++)
		logNoise("server1: transform for %s.%s language %s", transforms1[i].trftype.schemaname, transforms1[i].trftype.objectname,
				 transforms1[i].languagename);

	for (i = 0; i < ntransforms2; i++)
		logNoise("server2: transform for %s.%s language %s", transforms2[i].trftype.schemaname, transforms2[i].trftype.objectname,
	...
	while (i < ntransforms1 || j < ntransforms2)
	{
		...
		if (i == ntransforms1)
		{
			...
			dumpCreateTransform(fpre, &transforms2[j]); j++;
		...
		else if (compareNamesAndRelations(&transforms1[i].trftype, &transforms2[j].trftype, transforms1[i].languagename, transforms2[j].languagename) < 0)
		{
			logDebug("transform for %s.%s language %s: server1",
					 transforms1[i].trftype.schemaname, transforms1[i].trftype.objectname, transforms1[i].languagename);
			dumpDropTransform(fpost, &transforms1[i]);
			i++;
			qstat.transformremoved++;
		}
...
rdbcompTriggers()
{
	...
	for (i = 0; i < ntriggers1; i++) logNoise("server1: %s.%s", triggers1[i].table.schemaname, triggers1[i].table.objectname);
	for (i = 0; i < ntriggers2; i++) logNoise("server2: %s.%s", triggers2[i].table.schemaname, triggers2[i].table.objectname);
	...
	while (i < ntriggers1 || j < ntriggers2)
	{
		...
		if (i == ntriggers1)
		{
			logDebug("trigger %s.%s: server2", triggers2[j].table.schemaname, triggers2[j].table.objectname);
		...
		{
			logDebug("trigger %s.%s: server1", triggers1[i].table.schemaname, triggers1[i].table.objectname);
		...
		{
			logDebug("trigger %s.%s: server1 server2", triggers1[i].table.schemaname, triggers1[i].table.objectname);
...
rdbcompBaseTypes()
{
	...
	logLine1("COMPARE Base Types");
	logLine2("Base Types on server1");
	types1 = getBaseTypes(conn1, &ntypes1);
	logLine2("Base Types on server2");
	types2 = getBaseTypes(conn2, &ntypes2);

	for (i = 0; i < ntypes1; i++) logNoise("server1: %s.%s", types1[i].obj.schemaname, types1[i].obj.objectname);
	for (i = 0; i < ntypes2; i++) logNoise("server2: %s.%s", types2[i].obj.schemaname, types2[i].obj.objectname);
	...
	while (i < ntypes1 || j < ntypes2)
	{
		...
		if (i == ntypes1)
		{
			logDebug("type %s.%s: server2", types2[j].obj.schemaname, types2[j].obj.objectname);
			if (options.securitylabels) getBaseTypeSecurityLabels(conn2, &types2[j]);
		...
		{
			logDebug("type %s.%s: server1", types1[i].obj.schemaname, types1[i].obj.objectname);
		...
		{
			logDebug("type %s.%s: server1 server2", types1[i].obj.schemaname, types1[i].obj.objectname);
			if (options.securitylabels)
			{
...
rdbcompCompositeTypes()
{
	...
	logLine1("COMPARE Composite Types");
	logLine2("Composite Types on server1");
	types1 = getCompositeTypes(conn1, &ntypes1);
	logLine2("Composite Types on server2");
	...
	while (i < ntypes1 || j < ntypes2)
	{
		...
		{
			logDebug("type %s.%s: server2", types2[j].obj.schemaname, types2[j].obj.objectname);
			if (options.securitylabels) getCompositeTypeSecurityLabels(conn2, &types2[j]);
...
rdbcompEnumTypes()
{
	...
	logLine1("COMPARE Enum Types");
	logLine2("Enum Types on server1");
	types1 = getEnumTypes(conn1, &ntypes1);
	logLine2("Enum Types on server2");
	...
	while (i < ntypes1 || j < ntypes2)
	{
		...
		{
			logDebug("type %s.%s: server2", types2[j].obj.schemaname, types2[j].obj.objectname);
			if (options.securitylabels) getEnumTypeSecurityLabels(conn2, &types2[j]);
...
rdbcompRangeTypes()
{
	...
	logLine1("COMPARE Range Types");
	logLine2("Range Types on server1");
	types1 = getRangeTypes(conn1, &ntypes1);
	logLine2("Range Types on server2");
	...
	while (i < ntypes1 || j < ntypes2)
	{
		...
		{
			logDebug("type %s.%s: server2", types2[j].obj.schemaname, types2[j].obj.objectname);
			if (options.securitylabels) getRangeTypeSecurityLabels(conn2, &types2[j]);
...
rdbcompUserMappings()
{
	...
	logLine1("COMPARE User Mappings");
	logLine2("User Mappings on server1");
	usermappings1 = getUserMappings(conn1, &nusermappings1);
	logLine2("User Mappings on server2");
	usermappings2 = getUserMappings(conn2, &nusermappings2);

	for (i = 0; i < nusermappings1; i++) logNoise("server1: user(%s) server(%s)", usermappings1[i].user, usermappings1[i].server);
	for (i = 0; i < nusermappings2; i++) logNoise("server2: user(%s) server(%s)", usermappings2[i].user, usermappings2[i].server);
...
rdbcompViews()
{
	...
	logLine1("COMPARE Views");
	logLine2("Views on server1");
	views1 = getViews(conn1, &nviews1);
	logLine2("Views on server2");
	views2 = getViews(conn2, &nviews2);

	for (i = 0; i < nviews1; i++) logNoise("server1: %s.%s", views1[i].obj.schemaname, views1[i].obj.objectname);
	for (i = 0; i < nviews2; i++) logNoise("server2: %s.%s", views2[i].obj.schemaname, views2[i].obj.objectname);
	...
	while (i < nviews1 || j < nviews2)
	{
		...
		if (i == nviews1)
		{
			logDebug("view %s.%s: server2", views2[j].obj.schemaname, views2[j].obj.objectname);
			if (options.securitylabels) getViewSecurityLabels(conn2, &views2[j]);
		...
		{
			logDebug("view %s.%s: server1", views1[i].obj.schemaname, views1[i].obj.objectname);
		...
		{
			logDebug("view %s.%s: server1 server2", views1[i].obj.schemaname, views1[i].obj.objectname);
			if (options.securitylabels)
			{
...
printSummary(void)
{
	logLine1("SUMMARY OF CHANGES");
        fprintf(stderr, "------------------------------------------------------------\n");
        fprintf(stderr, "| ObjectType                     |      Added |    Removed |\n");
        fprintf(stderr, "|----------------------------------------------------------|\n");
        fprintf(stderr, "| %-30s | %10d | %10d |\n","Schema",qstat.schemaadded, qstat.schemaremoved);
	fprintf(stderr, "| %-30s | %10d | %10d |\n","Extension",qstat.extensionadded, qstat.extensionremoved);
	fprintf(stderr, "| %-30s | %10d | %10d |\n","Sequence",qstat.seqadded, qstat.seqremoved);
	fprintf(stderr, "| %-30s | %10d | %10d |\n","Table",qstat.tableadded, qstat.tableremoved);
	fprintf(stderr, "| %-30s | %10d | %10d |\n","Index",qstat.indexadded, qstat.indexremoved);
	fprintf(stderr, "| %-30s | %10d | %10d |\n","View",qstat.viewadded, qstat.viewremoved);
	fprintf(stderr, "| %-30s | %10d | %10d |\n","Materialized View",qstat.matviewadded, qstat.matviewremoved);

	fprintf(stderr, "| %-30s | %10d | %10d |\n","Type",qstat.typeadded, qstat.typeremoved);
	fprintf(stderr, "| %-30s | %10d | %10d |\n","Domain",qstat.domainadded, qstat.domainremoved);

	fprintf(stderr, "| %-30s | %10d | %10d |\n","Function",qstat.functionadded, qstat.functionremoved);
	fprintf(stderr, "| %-30s | %10d | %10d |\n","Procedure",qstat.procadded, qstat.procremoved);
	fprintf(stderr, "| %-30s | %10d | %10d |\n","Trigger",qstat.trgadded, qstat.trgremoved);
	fprintf(stderr, "| %-30s | %10d | %10d |\n","Statistics",qstat.stxadded, qstat.stxremoved);

	fprintf(stderr, "| %-30s | %10d | %10d |\n","Access Method",qstat.amadded, qstat.amremoved);
	fprintf(stderr, "| %-30s | %10d | %10d |\n","Policy",qstat.poladded, qstat.polremoved);
	fprintf(stderr, "| %-30s | %10d | %10d |\n","Rule",qstat.ruleadded, qstat.ruleremoved);

	fprintf(stderr, "| %-30s | %10d | %10d |\n","Aggregate",qstat.aggadded, qstat.aggremoved);

	fprintf(stderr, "| %-30s | %10d | %10d |\n","Language",qstat.languageadded, qstat.languageremoved);
	fprintf(stderr, "| %-30s | %10d | %10d |\n","Collation",qstat.collationadded, qstat.collationremoved);
	fprintf(stderr, "| %-30s | %10d | %10d |\n","Conversion",qstat.conversionadded, qstat.conversionremoved);
	fprintf(stderr, "| %-30s | %10d | %10d |\n","Event Trigger",qstat.evttrgadded, qstat.evttrgremoved);

	fprintf(stderr, "| %-30s | %10d | %10d |\n","FDW Server",qstat.serveradded, qstat.serverremoved);
	fprintf(stderr, "| %-30s | %10d | %10d |\n","Foreign Data Wrapper",qstat.fdwadded, qstat.fdwremoved);
	fprintf(stderr, "| %-30s | %10d | %10d |\n","User Mapping",qstat.usermappingadded, qstat.usermappingremoved);
	fprintf(stderr, "| %-30s | %10d | %10d |\n","Foreign Table",qstat.ftableadded, qstat.ftableremoved);

	fprintf(stderr, "| %-30s | %10d | %10d |\n","Cast",qstat.castadded, qstat.castremoved);
	fprintf(stderr, "| %-30s | %10d | %10d |\n","Operator",qstat.operatoradded, qstat.operatorremoved);
	fprintf(stderr, "| %-30s | %10d | %10d |\n","Operator Family",qstat.opfamilyadded, qstat.opfamilyremoved);
	fprintf(stderr, "| %-30s | %10d | %10d |\n","Operator Class",qstat.opclassadded, qstat.opclassremoved);

	fprintf(stderr, "| %-30s | %10d | %10d |\n","Publication",qstat.pubadded, qstat.pubremoved);
	fprintf(stderr, "| %-30s | %10d | %10d |\n","Subscription",qstat.subadded, qstat.subremoved);

	fprintf(stderr, "| %-30s | %10d | %10d |\n","Text Search Config",qstat.tsconfigadded, qstat.tsconfigremoved);
	fprintf(stderr, "| %-30s | %10d | %10d |\n","Text Search Dict",qstat.tsdictadded, qstat.tsdictremoved);
	fprintf(stderr, "| %-30s | %10d | %10d |\n","Text Search Parser",qstat.tsparseradded, qstat.tsparserremoved);
	fprintf(stderr, "| %-30s | %10d | %10d |\n","Text Search Template",qstat.tstemplateadded, qstat.tstemplateremoved);

	fprintf(stderr, "| %-30s | %10d | %10d |\n","Transform",qstat.transformadded, qstat.transformremoved);
        fprintf(stderr, "|----------------------------------------------------------|\n");
}
...
int main(int argc, char *argv[])
{
	...
	rdbcompGeneralOptions	gopts_given;	/* command-line informed? */
	rdbcompGeneralOptions	gopts;
	rdbcompDatabaseOptions	sopts;
	rdbcompDatabaseOptions	topts;
	...
	memset(&gopts_given, 0, sizeof(rdbcompGeneralOptions));
	memset(&gopts, 0, sizeof(rdbcompGeneralOptions));
	memset(&sopts, 0, sizeof(rdbcompDatabaseOptions));
	memset(&topts, 0, sizeof(rdbcompDatabaseOptions));
	...
	while ((c = getopt_long(argc, argv, "c:f:stv", long_options, &optindex)) != -1)
	{
		switch (c)
		{
			...
			case 't':
				...
				break;
			case 'v':
				if (loglevel == PGQ_ERROR) loglevel = PGQ_WARNING;
				else if (loglevel == PGQ_WARNING) loglevel = PGQ_DEBUG;
				else if (loglevel == PGQ_DEBUG) loglevel = PGQ_NOISE;
				break;
			case 0:
	 ...
	 */
	if (output_given) options.output = gopts.output;
	if (tmpdir_given) options.tmpdir = gopts.tmpdir;

	if (options.verbose && loglevel == PGQ_ERROR) loglevel = PGQ_DEBUG;
	logLine1("PROCESS OPTIONS");

	if (gopts_given.ignoreversion) options.ignoreversion = gopts.ignoreversion;
	logDebug("general.ignoreversion = %s", options.ignoreversion ? "true" : "false");

	if (gopts_given.summary) options.summary = gopts.summary;
	logDebug("general.summary = %s", options.summary ? "true" : "false");

	if (gopts_given.singletxn) options.singletxn = gopts.singletxn;
	logDebug("general.singletxn = %s", options.singletxn ? "true" : "false");

	if (gopts_given.aggregate) options.aggregate = gopts.aggregate;
	logDebug("general.aggregate = %s", options.aggregate ? "true" : "false");

	if (gopts_given.cast) options.cast = gopts.cast;
	logDebug("general.cast = %s", options.cast ? "true" : "false");

	if (gopts_given.collation) options.collation = gopts.collation;
	logDebug("general.collation = %s", options.collation ? "true" : "false");

	if (gopts_given.comment) options.comment = gopts.comment;
	logDebug("general.comment = %s", options.comment ? "true" : "false");

	if (gopts_given.conversion) options.conversion = gopts.conversion;
	logDebug("general.conversion = %s", options.conversion ? "true" : "false");

	if (gopts_given.domain) options.domain = gopts.domain;
	logDebug("general.domain = %s", options.domain ? "true" : "false");

	if (gopts_given.eventtrigger) options.eventtrigger = gopts.eventtrigger;
	logDebug("general.eventtrigger = %s", options.eventtrigger ? "true" : "false");

	if (gopts_given.extension) options.extension = gopts.extension;
	logDebug("general.extension = %s", options.extension ? "true" : "false");

	if (gopts_given.fdw) options.fdw = gopts.fdw;
	logDebug("general.fdw = %s", options.fdw ? "true" : "false");

	if (gopts_given.foreigntable) options.foreigntable = gopts.foreigntable;
	logDebug("general.foreigntable = %s", options.foreigntable ? "true" : "false");

	if (gopts_given.function) options.function = gopts.function;
	logDebug("general.function = %s", options.function ? "true" : "false");

	if (gopts_given.index) options.index = gopts.index;
	logDebug("general.index = %s", options.index ? "true" : "false");

	if (gopts_given.language) options.language = gopts.language;
	logDebug("general.language = %s", options.language ? "true" : "false");

	if (gopts_given.matview) options.matview = gopts.matview;
	logDebug("general.matview = %s", options.matview ? "true" : "false");

	if (gopts_given.operator) options.operator = gopts.operator;
	logDebug("general.operator = %s", options.operator ? "true" : "false");

	if (gopts_given.owner) options.owner = gopts.owner;
	logDebug("general.owner = %s", options.owner ? "true" : "false");

	if (gopts_given.privileges) options.privileges = gopts.privileges;
	logDebug("general.privileges = %s", options.privileges ? "true" : "false");

	if (gopts_given.procedure) options.procedure = gopts.procedure;
	logDebug("general.procedure = %s", options.procedure ? "true" : "false");

	if (gopts_given.rule) options.rule = gopts.rule;
	logDebug("general.rule = %s", options.rule ? "true" : "false");

	if (gopts_given.publication) options.publication = gopts.publication;
	logDebug("general.publication = %s", options.publication ? "true" : "false");

	if (gopts_given.schema) options.schema = gopts.schema;
	logDebug("general.schema = %s", options.schema ? "true" : "false");

	if (gopts_given.securitylabels) options.securitylabels = gopts.securitylabels;
	logDebug("general.securitylabels = %s", options.securitylabels ? "true" : "false");

	if (gopts_given.sequence) options.sequence = gopts.sequence;
	logDebug("general.sequence = %s", options.sequence ? "true" : "false");

	if (gopts_given.statistics) options.statistics = gopts.statistics;
	logDebug("general.statistics = %s", options.statistics ? "true" : "false");

	if (gopts_given.subscription) options.subscription = gopts.subscription;
	logDebug("general.subscription = %s", options.subscription ? "true" : "false");

	if (gopts_given.table) options.table = gopts.table;
	logDebug("general.table = %s", options.table ? "true" : "false");

	if (gopts_given.textsearch) options.textsearch = gopts.textsearch;
	logDebug("general.textsearch = %s", options.textsearch ? "true" : "false");

	if (gopts_given.transform) options.transform = gopts.transform;
	logDebug("general.transform = %s", options.transform ? "true" : "false");

	if (gopts_given.trigger) options.trigger = gopts.trigger;
	logDebug("general.trigger = %s", options.trigger ? "true" : "false");

	if (gopts_given.type) options.type = gopts.type;
	logDebug("general.type = %s", options.type ? "true" : "false");

	if (gopts_given.view) options.view = gopts.view;
	logDebug("general.view = %s", options.view ? "true" : "false");

	if (include_schema_given) options.include_schema = gopts.include_schema;
	logDebug("general.include_schema = %s", options.include_schema );

	if (exclude_schema_given) options.exclude_schema = gopts.exclude_schema;
	logDebug("general.exclude_schema = %s", options.exclude_schema );

	if (sopts.dbname) opts.source.dbname = sopts.dbname;

	if (sopts.host) opts.source.host = sopts.host;

	if (sopts.port) opts.source.port = sopts.port;

	if (sopts.username) opts.source.username = sopts.username;

	if (source_prompt_given) opts.source.promptpassword = sopts.promptpassword;

	if (topts.dbname) opts.target.dbname = topts.dbname;

	if (topts.host) opts.target.host = topts.host;

	if (topts.port) opts.target.port = topts.port;

	if (topts.username) opts.target.username = topts.username;

	if (target_prompt_given) opts.target.promptpassword = topts.promptpassword;

	logLine1("CONNECT");

	/* connecting to server1 ... */
	logDebug("target: db=%s host=%s port=%s user=%s", opts.target.dbname, opts.target.host, opts.target.port, opts.target.username);
	conn1 = connectDatabase(opts.target);
	logDebug("connected to target (server1)");
	...
	logDebug("source: db=%s host=%s port=%s user=%s", opts.source.dbname, opts.source.host, opts.source.port, opts.source.username);
	conn2 = connectDatabase(opts.source);
	logDebug("connected to source (server2)");
	...
	/*
	 * rdbcomp is using the reserved keywords provided by the postgres
	 ...
	 * old rdbcomp version or a to-be-released postgres version.  If you know
	...
	{
		logError("cannot connect to server whose version (%s) is greater than postgres version (%s) used to compile rdbcomp",
	...
	snprintf(prepath, PGQMAXPATH, "%s/rdbcomp.%d.pre", options.tmpdir, getpid());
	snprintf(postpath, PGQMAXPATH, "%s/rdbcomp.%d.post", options.tmpdir, getpid());
	...
	if (options.include_schema != NULL) {
		...
		logDebug("filter include schema: %s", include_schema_str);
	} else {
		include_schema_str = "";
	}
	if (options.exclude_schema != NULL) {
		exclude_schema_str = psprintf(" AND n.nspname !~ '%s'", options.exclude_schema);
		logDebug("filter exclude schema: %s", exclude_schema_str);
	} else {
		exclude_schema_str = "";
	}
	...
	if (options.language) rdbcompLanguages();
	if (options.schema) rdbcompSchemas();
	if (options.extension) rdbcompExtensions();
	if (options.accessmethod) rdbcompAccessMethods();
	if (options.cast) rdbcompCasts();
	if (options.collation) rdbcompCollations();
	if (options.conversion) rdbcompConversions();
	if (options.domain) rdbcompDomains();
	if (options.type) rdbcompTypes();
	if (options.operator) {
		rdbcompOperators();
		rdbcompOperatorFamilies();
		rdbcompOperatorClasses();
	}
	if (options.sequence) rdbcompSequences();
	if (options.table) rdbcompTables();
	if (options.index) rdbcompIndexes();
	if (options.function) rdbcompFunctions();
	if (options.procedure) rdbcompProcedures();
	if (options.foreigntable) rdbcompForeignTables();
	if (options.aggregate) rdbcompAggregates();
	if (options.view) rdbcompViews();
	if (options.matview) rdbcompMaterializedViews();
	if (options.trigger) rdbcompTriggers();
	if (options.rule) rdbcompRules();
	if (options.publication) rdbcompPublications();
	if (options.subscription) rdbcompSubscriptions();
	if (options.policy) rdbcompPolicies();
	if (options.eventtrigger) rdbcompEventTriggers();
	...
	if (options.transform) rdbcompTransforms();
	if (options.statistics) rdbcompStatistics();
	logLine1("PROCESS TEMP output");

	/* Print header iff there is at least one command. Check if one of the files is not empty. */
	fflush(fpre);
	fflush(fpost);
	...
	{
		fprintf(fout, "--\n-- rdbcomp %s\n", PGQ_VERSION);
		//fprintf(fout, "-- source version: %s\n", PQparameterStatus(conn1,"server_version") );
		//fprintf(fout, "-- target version: %s\n", PQparameterStatus(conn2,"server_version"));
	        fprintf(fout, "-- target: db=%s host=%s port=%s user=%s version=%s\n", 
				opts.target.dbname, opts.target.host, opts.target.port, opts.target.username,PQparameterStatus(conn1,"server_version"));
		fprintf(fout, "-- source: db=%s host=%s port=%s user=%s version=%s\n", 
				opts.source.dbname, opts.source.host, opts.source.port, opts.source.username,PQparameterStatus(conn2,"server_version"));
		fprintf(fout, "--");
	}

	/* execute as a single transaction */
	if (options.singletxn && (!isEmptyFile(prepath) || !isEmptyFile(postpath))) fprintf(fout, "\n\nBEGIN;");

	/* dump the rdbcomp in the right order */
