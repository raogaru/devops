...
#define logLine1(...) do { \
	logGeneric(PGQ_DEBUG, LINE1); \
	logGeneric(PGQ_DEBUG, __VA_ARGS__); \
	} while (0)

#define logLine2(...) do { \
	logGeneric(PGQ_DEBUG, LINE2); \
	logGeneric(PGQ_DEBUG, __VA_ARGS__); \
	} while (0)


typedef struct rdbcompGeneralOptions
{
	char			*output;
	char			*tmpdir;
	bool			ignoreversion;
	...
	char			*include_schema;
	char			*exclude_schema;
} rdbcompGeneralOptions;

typedef struct rdbcompDatabaseOptions
{
	char			*host;
	char			*port;
	char			*username;
	...
	bool			istarget;
	bool			promptpassword;
} rdbcompDatabaseOptions;

typedef struct rdbcompOptions
{
	rdbcompGeneralOptions	general;
	rdbcompDatabaseOptions	source;
	rdbcompDatabaseOptions	target;
} rdbcompOptions;
...
extern rdbcompGeneralOptions options;
