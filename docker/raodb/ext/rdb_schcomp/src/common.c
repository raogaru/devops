...
static stringListCell *intersectWithSortedLists(stringListCell *a, stringListCell *b, bool withvalue, bool changed);
static stringListCell *setDifferenceWithSortedLists(stringListCell *a, stringListCell *b, bool withvalue);
...
compareRelations(PQLObject *a, PQLObject *b)
{
	...
	if (c == 0) c = strcmp(a->objectname, b->objectname);
	return c;
}
 ...
 * version that rdbcomp was compiled with. Ensure that rdbcomp is compiled
