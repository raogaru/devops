...
# ----------------------------
# SQL generation (NAMED PARAMS)
# ----------------------------
def make_sql(proc: ProcDef) -> str:
    """
    Build either positional or named-parameter call with explicit casts.
    Named form (recommended for your request):
      CALL sch.p(p_a => $1::int4, p_b => $2::text)
      SELECT sch.f(p_a => $1::int4, p_b => $2::text)
    """
    if proc.param_style not in {"named", "positional"}:
        raise ValueError(f"{proc.name}: invalid param_style={proc.param_style}")

    parts = []
    for i, p in enumerate(proc.params, start=1):
        if proc.param_style == "named":
            parts.append(f"{p.name} => ${i}::{p.type}")
        else:
            parts.append(f"${i}::{p.type}")
...
def load_samples_csv(samples_dir: str, proc: ProcDef) -> List[Dict[str, str]]:
    ...
    if not os.path.exists(path):
        raise FileNotFoundError(f"Samples file not found for {proc.name}: {path}")
...
def row_to_args(proc: ProcDef, row: Dict[str, str]) -> List[Any]:
    # IMPORTANT: args order must match YAML params order (even for named calls, because $1..$N)
...
def main() -> int:
    ap = argparse.ArgumentParser(description="PostgreSQL named-parameter procedure/function benchmark runner")
    ...
    for proc in procs:
        ...
        print(f"    style={proc.param_style} | SQL: {sql}")
        print(f"    samples={len(samples)} warmup={args.warmup} iterations={args.iterations}")
        ...
        if s:
            print(f"    OK={len(ok_durations)} ERR={len(bad)} | "
