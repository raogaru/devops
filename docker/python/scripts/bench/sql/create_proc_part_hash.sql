...
LANGUAGE plpgsql AS $$
BEGIN
    OPEN p_refcur FOR SELECT * FROM bench.t_hash1 LIMIT (floor(random()*99 + 1)::int);
END;
$$; 
...
LANGUAGE plpgsql AS $$
BEGIN
    OPEN p_refcur FOR SELECT * FROM bench.t_hash2 LIMIT (floor(random()*99 + 1)::int);
END;
$$; 
...
LANGUAGE plpgsql AS $$
BEGIN
    OPEN p_refcur FOR SELECT * FROM bench.t_hash3 LIMIT (floor(random()*99 + 1)::int);
END;
$$; 
