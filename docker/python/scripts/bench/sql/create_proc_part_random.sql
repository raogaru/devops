...
LANGUAGE plpgsql AS $$
BEGIN
    OPEN p_refcur FOR SELECT * FROM bench.t_random1 LIMIT (floor(random()*99 + 1)::int);
END;
$$; 
...
LANGUAGE plpgsql AS $$
BEGIN
    OPEN p_refcur FOR SELECT * FROM bench.t_random2 LIMIT (floor(random()*99 + 1)::int);
END;
$$; 
...
LANGUAGE plpgsql AS $$
BEGIN
    OPEN p_refcur FOR SELECT * FROM bench.t_random3 LIMIT (floor(random()*99 + 1)::int);
END;
$$; 
