...
CONSUMER_LOG = os.getenv("CONSUMER_LOG", "/data/consumer.log")
...
def ensure_dirs():
    ...
    for d in [INCOMING_DIR, PROCESSING_DIR, PROCESSED_DIR, FAILED_DIR, os.path.dirname(CONSUMER_LOG)]:
...
def append_to_log(filename, status, bytes_count):
    ...
    ensure_dirs()
    file_exists = os.path.exists(CONSUMER_LOG)
    with open(CONSUMER_LOG, "a", newline="") as f:
        writer = csv.writer(f)
        if not file_exists:
...
def process_file(filepath):
    ...
    try:
        ...
        with open(filepath, "r") as f:
            content = f.read()
            logging.info(f"file={filepath} bytes-read={len(content)}")
...
def handle_file(filename):
    ...
    if not is_file_ready(src):
        logging.info(f"file={filename} skipping as still being generated")
        return
    ...
    logging.info(f"file={filename} status={status} moved-to={dest_dir}")
...
def run_once():
    ...
    ensure_dirs()
    logging.info('=' * 80)
    logging.info("CSV Consumer scheduled run started.")
    ...
    logging.info("✅ CSV-Consumer run completed.")
    logging.info('=' * 80)
