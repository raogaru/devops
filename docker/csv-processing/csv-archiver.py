...
ARCHIVER_LOG = os.getenv("ARCHIVER_LOG", "/data/archiver.log")
...
def ensure_dirs():
    ...
    os.makedirs(os.path.dirname(ARCHIVER_LOG), exist_ok=True)

def append_to_log(filename, age_seconds):
    """Append deletion record to /data/archiver.log"""
    ensure_dirs()
    file_exists = os.path.exists(ARCHIVER_LOG)
    with open(ARCHIVER_LOG, "a", newline="") as logf:
...
if __name__ == "__main__":
    ...
    logging.info('=' * 80)
    logging.info(
        f"CSV Archiver started. Checking {PROCESSED_DIR} every {INTERVAL_SECONDS}s; "
