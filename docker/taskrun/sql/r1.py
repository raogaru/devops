...
def run_group(dsn, group, defaults, execution_cfg):
    ...
    # -----------------------------------------
    if group.get("skip", False):
        print(f"    Group {group['name']} SKIPPED")
        group_start_time = datetime.now()
        group_end_time = group_start_time
        print(f"    Group {group['name']} START TIME : {group_start_time}")
        print(f"    Group {group['name']} END TIME   : {group_end_time}")
        print(f"    Group {group['name']} DURATION   : 0.00s")
        return
